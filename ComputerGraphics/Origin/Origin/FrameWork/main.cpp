...
{	
    ...
    Fl_File_Chooser fileChoose(".", "*.bvh", Fl_File_Chooser ::SINGLE, "BVH file selector");
    
    fileChoose.show();

    while (fileChoose.shown())
    {
        Fl::wait();
    }
    if (fileChoose.count() == 1)
    {
        win->m_bvh->Load(fileChoose.value());
        win->m_slider->bounds(0, win->m_bvh->GetNumFrame());
    }
